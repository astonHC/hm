    ...
    document.addEventListener('DOMContentLoaded',function populateBasket() {
        for (let i of listItems) {
            if (!document.querySelector(`#quantity-input-${i.id}`)) {
                ...
                item.className = 'bg-amber rounded-lg mb-10 p-10 flex justify-between items-center xs:p-0';
                item.id=`${i.id}`
                item.innerHTML =
                    `<span class="item-img-text-link flex text-wrap"><img class="w-[100px] h-[100px]"
                            src=${i.img} width="130px"
                            height="130px" alt="IMAGE" />
                        <div class="ml-3 max-w-[50%]"><a class="text-2xl text-nowrap">${i.name}</a>
                            <p id="item-description" class="text-base leading-5 w-[100%]">${i.description}</p>
                            <p id="item-price" class="text-base font-bold leading-9">£${i.price}</p>
                        </div>
                    </span>
                    ...
                    <input type="checkbox" checked="true" onclick="getTotal()"
                        class="isinOrder bg-yellow-100 dark:bg-yellow-700 dark:hover:bg-yellow-600 dark:border-yellow-600 hover:bg-yellow-200 border checked:bg-yellow-700 focus:bg-yellow-700 active:bg-yellow-700 ml-8 mt-auto mb-auto" />
                    </div>
                    `
...
<body >
    ...
    <div class="flex flex-wrap lg:flex-nowrap" onload="populateBasket()">
        ...
        <div
            ...
            <button class="bg-amber rounded-lg mb-10 p-4 mr-5 ml-5 mt-10 w-full h-56 lg:h-24 text-7xl lg:text-3xl"
